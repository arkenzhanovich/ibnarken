...
#Версия скрипта
scriptversion = "ARU_FHL_v020425"

#Глобальные переменные
print(col_yb + f"{scriptversion}:")
print (col_y + f"Описание" + Fore.RESET)
print(col_g + f"[Смещение]: " +Fore.RESET + f"смещение времени подачи заявки в мс")
print(col_g + f"[Токен]: " + Fore.RESET + f"войдите в аккаунт на https://c.mi.com/global, вставьте код в адресную строку браузера, удалите пробел после "'"javascript"'" и нажмите "'"Enter"'". Можно вместо кода воспользоваться Cookie Editor (значения для токена в поле " + col_b + f""'"new_bbs_serviceToken"'"" + Fore.RESET + " или" + col_b +  f" "'"popRunToken"'"" + Fore.RESET + f". Код:\n")
print (col_b + "javascript :(function(){var token=document.cookie.match(/popRunToken=([^;]+)/);if(token){prompt("'"Copy the token:"'", token[1]);}else{alert("'"Token not found"'");}})()" + Fore.RESET)
  
print (col_y + f"\nДанные для работы скрипта и проверка статуса аккаунта" + Fore.RESET)
cookie_value = input(col_g + f"[Токен]: " + Fore.RESET)
feed_time_shift = float(input (col_g + f"[Смещение]: " + Fore.RESET ))
...
def get_initial_beijing_time():
    ...
    for server in ntp_servers:
        try:
            print(col_yb + f"\n{scriptversion}:")
            print(col_y + f"Вычисление Пекинского времени" + Fore.RESET)
...
def wait_until_target_time(start_beijing_time, start_timestamp):
    ...
    print(col_y + f"Подача заявки на разблокировку загрузчика" + Fore.RESET)
    print (col_g + f"[Смещение]: " + Fore.RESET + f"{feed_time_shift:.2f} мс.")
    ...
    try:
        ...
        if is_pass == 4:
            if button_state == 1:
                    ...
                    statcosolclear = (input(f"Очистить консоль (" + col_b + f"Yes/No" +Fore.RESET + f")?: ") )
                    if (statcosolclear == 'y' or statcosolclear == 'Y' or statcosolclear == 'yes' or statcosolclear == 'Yes' or statcosolclear == 'YES'):
...
def main():
    print (col_g + f"[Заданные параметры]: " + Fore.RESET + f"смещение времени подачи заявки {feed_time_shift:.2f} мс.")
